...
function CaerdonProfessionMixin:GetProfessionInfo()
    ...
    local hasSkillLine, meetsMinRank, rank, maxRank = CaerdonRecipe:GetPlayerSkillInfo(self.item:GetItemSubType(), 0)
        ...
        if not needsItem then
            ...
            elseif inventoryType == Enum.InventoryType.IndexProfessionGearType then
                if #accessoryGears == 0 then
                    ...
                    else -- no matching item but slots are filled - mark it as an option?
                    end
                end
            end
        end
